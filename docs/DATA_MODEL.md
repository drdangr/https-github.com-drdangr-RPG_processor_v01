...
```typescript
  ...
  connectionId: string // ID локации ИЛИ объекта, где находится персонаж (унифицированная связь)
  locationId?: string  // ID корневой локации (вычисляется автоматически)
  attributes: Record<string, string>; // Нарративные характеристики
}
```

> **Важно:** Теперь игроки используют `connectionId` так же, как и объекты. Это позволяет помещать персонажей внутрь объектов (машина, шкаф, кресло).

> **locationId:** Это поле автоматически вычисляется и обновляется при перемещении персонажа. Оно всегда содержит ID корневой локации, даже если персонаж находится внутри объекта. Например, если персонаж сидит в машине (`connectionId: "obj_car"`), а машина находится в гараже (`obj_car.connectionId: "loc_garage"`), то `locationId` будет `"loc_garage"`.
...
Перемещение персонажа = изменение его `connectionId`.

* **Глобальная навигация:** Для перехода между локациями (из Комнаты в Коридор) используется `move_player`.
* **Взаимодействие с объектами:** Для входа в объекты (сесть в машину, спрятаться в шкаф) используется `move_object`.

```typescript
// Джек переходит в салун (move_player)
char_jack.connectionId = "loc_saloon"

// Джек садится в машину (move_object)
char_jack.connectionId = "obj_car"
```
...
### Автоматическое обновление locationId

При перемещении объектов с помощью `move_object` или drag-and-drop в UI:

* Все персонажи внутри перемещаемого объекта автоматически получают обновленный `locationId`
* `locationId` вычисляется путем обхода иерархии вверх до корневой локации
* Это гарантирует, что система всегда знает, в какой локации физически находится персонаж
