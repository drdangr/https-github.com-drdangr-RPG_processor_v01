  ...
  definition: {
    ...
    parameters: {
      ...
      properties: {
        ...
        attributes: { 
          type: Type.STRING,
          description: "JSON объект с атрибутами в формате {\"ключ\": \"значение\"}. Примеры атрибутов: condition (состояние), type (тип), size (размер), material (материал), appearance (внешний вид), smell (запах), content (содержимое), feature (особенность), quality (качество), durability (прочность). Все значения должны быть нарративными описаниями. ВАЖНО: Для исчисляемых значений (количество предметов, патронов, монет и т.д.) ВСЕГДА используй цифры, а не слова. Например: \"6 патронов крупного калибра\" вместо \"шесть патронов крупного калибра\", \"3 монеты\" вместо \"три монеты\". Пример: {\"condition\": \"ржавый\", \"size\": \"небольшой\", \"material\": \"железный\", \"content\": \"6 патронов крупного калибра\"}"
        },
      },
